...
static int queue_picture(MainState *is, AVFrame *src_frame, double pts, double duration, int64_t pos, int serial){
    Frame* vp;
    ...
    if (!(vp = frame_queue_peek_writable(&is->pickq)))
        return -1;
